...
class CircleChanger(object):
    ...
    def swallow(self, other_circle_changer):
        ...
        new_circle = rg.Circle(self.circle.center.halfway_to(
            other_circle_changer.circle.center),
            self.circle.center.get_distance_from(
                other_circle_changer.circle.center)/2)
        new_circle_changer = CircleChanger(new_circle.center.x,
                                          new_circle.center.y,
                                          new_circle.radius,
                                          'red', self.colors +
    ...
    def change_to_next_color_in_tuple(self):
        ...
        self.circle.fill_color = self.colors[self.change_color_index]

        if self.change_color_index == (len(self.colors) - 1):
            self.change_color_index = 0
        else:
            self.change_color_index += 1
